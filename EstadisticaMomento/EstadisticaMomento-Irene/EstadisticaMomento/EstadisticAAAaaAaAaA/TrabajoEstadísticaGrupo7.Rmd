...
Vamos a hacer tablas de frecuencias para ver la frecuencia en algunos de los datos de el dataset. Los datos que mediremos en estas tablas seran: el dato 'tag' que da información sobre el juego, el dato 'to_beat_main' que da información sobre el tiempo en horas que se requiere para terminar la historia principal del juego, el dato 'to_beat_extra' que da informaciçon sobre el tiempo en horas para completar contenido extra y el dato 'to_beat_completionist' que da información del tiempo en horas para completar todos los objetivos del juego.

#### Tablas de frecuencia tags
```{r}
...
#### Tablas de frecuencia to-beat-main
```{r}
# Paso 3: Crear una tabla de frecuencias
...
```
#### Tablas de frecuencia to-beat-extra
```{r}
# Paso 3: Crear una tabla de frecuencias
...
```
#### Tablas de frecuencia to-beat-completionist
```{r}
# Paso 3: Crear una tabla de frecuencias
...
####Gráfico de barras juegos sacados por año
```{r}
datos |> 
...
####Gráfico de barras Metacritic rating (20-100)
```{r}
datos |> 
  # Inicia una visualización de ggplot con 'lab' como el dataframe y 'tipo' como variable en el eje x.
...
#####Resumen Metacritic rating (20-100)
```{r}
...
####Gráfico de barras Reviewer rating (0-10)
```{r}
datos |> 
  # Inicia una visualización de ggplot con 'lab' como el dataframe y 'tipo' como variable en el eje x.
...
###Gráfico de densidad positivity_ratio

```{r}
# Gráfico de densidad de positivity_ratio
ggplot(datos, aes(x = positivity_ratio)) +
  geom_density(fill = "skyblue", alpha = 0.6) +  # Rellena el área bajo la curva
  theme_minimal() +  # Usa un tema minimalista
  labs(title = "Distribución de Positivity Ratio",
       x = "Positivity Ratio",
       y = "Densidad") +
  scale_x_continuous(limits = c(0, 25))  # Ajusta los límites del eje x si es necesario
```
El positivity_ratio, como mencionado previamente, es el cociente entre las valoraciones positivas y las valoraciones negativas. Aquí lo podemos ver representado en un gráfico de densidad en donde podemos observar un pico prominente alrededor del 2 realizando una distribución unimodal.El gráfico además tiene una cola larga indicando valores extremos poco frecuentes.Se ha decidido realizar el gráfico entre valores (0,25) aún habiendo valores mucho más superiores ya que no son suficientemente frecuentes para hacer un impacto significativo en el análisis.  


###Gráfico de frequencia de cada tag
```{r}
...
###Gráfico de barras Metacritic y Rewiewer ratings
...
###Gráfico de dispersión Metacritic y Rewiewer ratings
...
###Gráfico de dispersión de puntuaciones según el tiempo para completar el juego.
...
ggplot(datos, aes(x = to_beat_main, y = metacritic_rating)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
### Gráfico de dispersión de to_beat_extra y to_beat_completionist:
...
ggplot(datos_filtrados_dispersion, aes(x = to_beat_extra, y = to_beat_completionist)) +
  ...
  labs(title = "Gráfico de Dispersión: Tiempo para Completar Contenido Extra vs. Completar Todo el Juego",
...
ggplot(datos_filtrados_grouped, aes(x = to_beat_extra_grouped, y = to_beat_completionist_grouped)) +
  geom_point(color = "blue", alpha = 0.5) +  # Agregar puntos de dispersión
  labs(title = "Gráfico de Dispersión Agrupado: Tiempo para Completar Contenido Extra vs. Completar Todo el Juego",
...
### Análisis de dispersión positivity_ratio y to_beat_main
A continuación vamos a analizar el ratio de positividad junto con el tiempo que se tarda en jugar la campaña principal/inicial. Primero creamos el gráfco de tendencia y calculamos su correlación:

```{r}
# Paso 1: Agrupar los datos en intervalos para to_beat_main
# Definimos intervalos para el tiempo de completar el juego principal
datos <- datos %>%
  mutate(to_beat_main_bins = cut(to_beat_main, 
                                  breaks = seq(0, 300, by = 20), 
                                  include.lowest = TRUE, 
                                  right = FALSE))

# Paso 2: Calcular el promedio de positivity_ratio por cada intervalo
tendencias <- datos %>%
  group_by(to_beat_main_bins) %>%
  summarise(promedio_positivity_ratio = mean(positivity_ratio, na.rm = TRUE))

# Paso 3: Visualizar la tendencia
ggplot(tendencias, aes(x = to_beat_main_bins, y = promedio_positivity_ratio)) +
  geom_point(color = "skyblue", size = 3) +  # Puntos de dispersión
  geom_line(color = "red", size = 1) +    # Línea de tendencia
  theme_minimal() +
  labs(title = "Tendencia del Positivity Ratio según Tiempo para Completar el Juego Principal",
       x = "Tiempo para Completar el Juego Principal (horas)",
       y = "Promedio de Positivity Ratio") +
  scale_x_discrete(breaks = levels(tendencias$to_beat_main_bins)) +  # Asegurar que los intervalos se muestran adecuadamente
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Mejora la legibilidad de las etiquetas del eje x
```
```{r}
# Calcular la correlación entre positivity_ratio y to_beat_main
correlacion <- cor(datos$positivity_ratio, datos$to_beat_main, use = "complete.obs")

# Mostrar el resultado
print(paste("El coeficiente de correlación entre positivity_ratio y to_beat_main es:", correlacion))
```
Como se puede observar la correlación se puede redondear como 0, por tanto no existe una correlación entre ambas variables ya que ni aumentan ambas a la vez ni disminuyen juntas. Esto también se puede observar en el gráfico de tendencia ya que esta no parece seguir ningún tipo de tendencia. La falta de correlación y tendencia puede sugerir que factores adicionales influyen más en el positivity_ratio que el tiempo de juego.
