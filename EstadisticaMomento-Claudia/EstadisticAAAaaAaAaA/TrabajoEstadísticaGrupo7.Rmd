...
!!!!!!!!!!!!!!!!!!!!!!!!!!! Párrafo aclaratorio de variables id y name.
...
```
Este resultado indica que no hay una gran relación lineal pese a que la covarianza era bastante positiva y, aunque existe una leve tendencia a que las calificaciones de los críticos suelan aumentear con los años debido a una mejora en la industria pues cada vez se crearían proyectos mejores que terminan gustando más, esta relación es muy débil y probablemente no significativa por lo que no tiene nada que ver.
...
### Análisis de 'metacritic_rating' y 'to_beat_main'
```{r}
# Filtrar los datos para mantener solo juegos con un tiempo de completar entre 0 y 100 horas
datos_filtrados <- datos %>%
  filter(to_beat_main >= 0 & to_beat_main <= 100)
...
A continuación, se comparará el año de salida de los juegos con el género de juego más producido cada año.
...
### Análisis de 'to_beat_extra' y 'to_beat_completionist'
// Este apartado se va a ir a tomar por el culo y hay q sustituirlo por 'tags' y 'metacritic_ratin' y lo mismo con 'reviewer_rating'
...
# Conclusión 
//Irene con sueño al habla, tenemos que leer todos los analisis bivariantes y ver que cómo afectan al mercado, o que tags son mas usados, etc. Recordad mirar lo de la tremenda tabla que nos sale en el html, besitos y a mimir

Tras el completo análisis de los datos de estos últimos años en cuanto a juegos en la plataforma de Steam, se han encontrado diversos factores que verdaderamente afectan a la opinión del público o que categoría de juegos ha sido mayor tendencia en estas últimas décadas.

// Claudia al habla equisde, por si acaso luego no me puedo meter a la conclusión, de mi parte lo más destacable es que las puntuaciones de metacritic y reviewers no suelen variar demasiado. Por otro lado que la duracion de un juego no es un factor que influya en su puntuación tanto en el metacritic como en el review (btw todo esto lo redacté en mi parte asi que no se si es una conclusión)
