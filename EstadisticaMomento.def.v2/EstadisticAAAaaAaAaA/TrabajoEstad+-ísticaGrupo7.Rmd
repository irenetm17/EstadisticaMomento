...
* `positivity_ratio`: Ratio de positividad, cociente entre las valoraciones positivas y las valoraciones negativas (numérica continua). No se va a usar.
...
* `extra_content_length`: Diferencia de tiempo entre el contenido principal y el extra (numérica continua, medida en horas). No se va a usar.
...
Se observa cómo la producción de los videojuegos lleva en auge las últimas décadas, haciendo un salto en 2015, además de cómo se frenó la caída de la industria en el año 2020 debido a la pandemia mundial y a la cuarentena. 
...
datos_filtrados_metacritic <- datos %>%
...
breaks <- seq(20, max(datos_filtrados_metacritic$metacritic_rating, na.rm = TRUE), length.out = 8)  # 7 intervalos

# Corta los datos en los intervalos especificados
metacritic_rating <- cut(datos_filtrados_metacritic$metacritic_rating, breaks = breaks)
...
La información se amplía con este resumen en el que se destaca que la puntuación media son 6,14 puntos. La nota más baja es 1 punto, algo que en el gráfico no se aprecia con claridad. La más alta son 9 puntos. Al igual que en ocurría con 'metacritic_rating', no hay ningún juego con la puntuación máxima, en este caso, 10.

### 4.1.4. Variable 'to_beat_main'
Tabla de frecuencias dividida en 10 intervalos:
```{r}
# Definir intervalos
Tiempo <- cut(datos$to_beat_main, breaks = 10)  # Definimos intervalos
...
Como en la tabla de frecuencias no se aprecian bien los intervalos debido a la inmensa cantidad de valores, se ha seleccionado un rango de 0 a 50 horas, que es donde se encuentran la mayoría de valores.

```{r}
# Filtrar los datos para eliminar valores NA
datos_filtrados_main1 <- datos %>%
  filter(!is.na(to_beat_main))

# Truncar los valores mayores a 50
datos_filtrados_main1$to_beat_main <- ifelse(datos_filtrados_main1$to_beat_main > 50, 51, datos_filtrados_main1$to_beat_main)
...
Tiempo <- cut(datos_filtrados_main1$to_beat_main, 
...
# Filtrar los datos para eliminar valores NA
datos_filtrados_main2 <- datos %>%
  filter(!is.na(to_beat_main))

# Crear el gráfico de densidad
ggplot(datos_filtrados_main2, aes(x = to_beat_main)) +
...
datos_filtrados_main2 <- datos_filtrados_main2 %>%
...
ggplot(datos_filtrados_main2, aes(x = to_beat_main_grouped)) +
  geom_density(fill = "skyblue", color = "coral1", alpha = 0.6) +
...
```
La información se amplía y precisa con este resumen en el que se destaca que el tiempo medio es de 9,49 horas. El tiempo más bajo es de 0,01 horas y el más alto, de 2105,84 horas. Por otro lado, la mediana se encuentra en las 4 horas.
...
Dado que en la tabla de frecuencias no se aprecian bien los intervalos debido a la inmensa cantidad de valores, se ha seleccionado un rango de 0 a 50 horas, que es donde se encuentran la mayoría de valores, tal y como ocurría con el caso anterior.

```{r}
# Filtrar los datos para eliminar valores NA
datos_filtrados_extra1 <- datos %>%
  filter(!is.na(to_beat_extra))

# Truncar los valores mayores a 50
datos_filtrados_extra1$to_beat_extra <- ifelse(datos_filtrados_extra1$to_beat_extra > 50, 51, datos_filtrados_extra1$to_beat_extra)

# Definir los intervalos en el rango de 0 a 50 horas, más un intervalo adicional para valores mayores a 50
Tiempo <- cut(datos_filtrados_extra1$to_beat_extra, 
              breaks = c(seq(0, 50, length.out = 11), Inf),   # Inf para incluir valores mayores a 50
              labels = etiquetas_intervalos, 
              right = FALSE)  # Intervalos cerrados a la izquierda y abiertos a la derecha
...
# Filtrar los datos para eliminar valores NA
datos_filtrados_extra2 <- datos %>%
  filter(!is.na(to_beat_extra))

# Crear el gráfico de densidad
ggplot(datos_filtrados_extra2, aes(x = to_beat_extra)) +
  geom_density(fill = "skyblue", color = "coral1", alpha = 0.6) +
...
Tal y como ocurría en el análisis de la variable anterior, la existencia de muy pocos datos con valores muy grandes dificulta el análisis del gráfico de densidad. Consecuentemente, se ha optado por hacer otro gráfico de densidad agrupando los datos más grandes, a partir de 50 horas, en un mismo valor. De este modo, es posible analizar con mayor precisión los datos en este rango de 0 a 50 horas, que es donde se encuentran la mayoría de ellos. Al aplicar este cambio, se obtiene el siguiente gráfico de densidad:

```{r density_plot_grouped_extra2, echo=TRUE}
# Agrupar valores mayores a 50 horas en una nueva categoría
datos_filtrados_extra2 <- datos_filtrados_extra2 %>%
  mutate(to_beat_extra_grouped = ifelse(to_beat_extra > 50, 50, to_beat_extra))

# Crear el gráfico de densidad usando la variable agrupada
ggplot(datos_filtrados_extra2, aes(x = to_beat_extra_grouped)) +
  geom_density(fill = "skyblue", color = "coral1", alpha = 0.6) +
...
```
La información se amplía y precisa con este resumen en el que se destaca que el tiempo medio es de 20 horas. El tiempo más bajo es de 0,03 horas y el más alto, de 6500 horas. Por otro lado, la mediana se encuentra en las 7,5 horas.
...
```
De nuevo, como en la tabla de frecuencias no se aprecian bien los intervalos debido a la inmensa cantidad de valores, se ha seleccionado un rango de 0 a 50 horas, que es donde se encuentran la mayoría de valores.

```{r}
# Filtrar los datos para eliminar valores NA
datos_filtrados_completionist1 <- datos %>%
  filter(!is.na(to_beat_completionist))

# Truncar los valores mayores a 50
datos_filtrados_completionist1$to_beat_completionist <- ifelse(datos_filtrados_completionist1$to_beat_completionist > 50, 51, datos_filtrados_completionist1$to_beat_completionist)

# Definir los intervalos en el rango de 0 a 50 horas, más un intervalo adicional para valores mayores a 50
Tiempo <- cut(datos_filtrados_completionist1$to_beat_completionist, 
              breaks = c(seq(0, 50, length.out = 11), Inf),   # Inf para incluir valores mayores a 50
              labels = etiquetas_intervalos, 
              right = FALSE)  # Intervalos cerrados a la izquierda y abiertos a la derecha
...
# Filtrar los datos para eliminar valores NA
datos_filtrados_completionist2 <- datos %>%
  filter(!is.na(to_beat_completionist))

# Crear el gráfico de densidad
ggplot(datos_filtrados_completionist2, aes(x = to_beat_completionist)) +
  geom_density(fill = "skyblue", color = "coral1", alpha = 0.6) +
...
Similar a los casos anteriores, la existencia de muy pocos datos con valores muy grandes dificulta el análisis del gráfico de densidad. Por lo tanto, se ha optado por hacer otro gráfico de densidad agrupando los datos más grandes, a partir de 50 horas, en un mismo valor. De este modo, es posible analizar con mayor precisión los datos en este rango de 0 a 50 horas, que es donde se encuentran la mayoría de ellos. Al aplicar este cambio, se obtiene el siguiente gráfico de densidad:

```{r density_plot_grouped_extra3, echo=TRUE}
# Agrupar valores mayores a 50 horas en una nueva categoría
datos_filtrados_completionist2 <- datos_filtrados_completionist2 %>%
  mutate(to_beat_completionist_grouped = ifelse(to_beat_completionist > 50, 50, to_beat_completionist))

# Crear el gráfico de densidad usando la variable agrupada
ggplot(datos_filtrados_completionist2, aes(x = to_beat_completionist_grouped)) +
  geom_density(fill = "skyblue", color = "coral1", alpha = 0.6) +
...
A partir de estos dos gráficos, entonces, es posible establecer una clara predominancia de juegos cuyo contenido completo tan solo requieren entre 0 y 7,5 horas para completarse, con un pico claro en unas 2,5 horas. A partir de entonces, la densidad de juegos no hace más que decrecer a medida que aumenta la variable del tiempo. Ciertamente, en este último gráfico se puede observar que la densidad vuelve a crecer a partir de 46 aproximadamente, pero eso se debe a la agrupación de esos datos cuyo tiempo es mayor a 50. No obstante, si se observa con detenimiento el primer gráfico de densidad, se observa que en realidad, la densidad de juegos a partir de las 50 horas se mantiene bastante estable.
...
Como se puede comprobar, la cantidad de 'tags' es tal que la gráfica no es capaz de mostrarlas con claridad. Debido a esto, se llevarán a cabo dos gráficas más, una comprobará la frecuencia de los 25 'tags' que lideran la gráfica anterior y, la otra, la frecuencia de las menos utilizadas.
...
A modo de introducción, un breve resumen de las correlaciones entre los pares de datos que se analizarán. 
...
```
Posteriormente se analizará en detalle qué significa cada resultado, pero para una mejor visualización, aquí una tabla y el gráfico de correlaciones a modo de resumen.

```{r}
# Crear un data frame con las correlaciones
correlations_df <- data.frame(
  Comparación = c("'year' vs 'metacritic_rating'",
                  "'reviewer_rating' vs 'metacritic_rating'",
                  "'reviewer_rating' vs 'to_beat_main'",
                  "'metacritic_rating' vs 'to_beat_main'"),
...
A continuación, se comparará el año de salida de los juegos con el género de juego más producido cada año, seguido de un resumen en tabla de lo que muestra el gráfico.
...
datos_filtrados_main3 <- datos %>% filter(to_beat_main <= 50)

# Generar el boxplot con los datos filtrados
ggplot(datos_filtrados_main3, aes(x = factor(tags_freq), y = to_beat_main)) + 
  geom_boxplot() +
...
# Filtrar valores por debajo de un umbral de 50 horas
datos_filtrados_extra3 <- datos %>% filter(to_beat_extra <= 50)

# Generar el boxplot con los datos filtrados
ggplot(datos_filtrados_extra3, aes(x = factor(tags_freq), y = to_beat_extra)) + 
  geom_boxplot() +
...
En este segundo gráfico se puede observar cómo los valores ascienden un poco estando entre las 3 y las 15 horas.
```{r}

datos$tags_freq <- sample(1:25, 63543, replace = TRUE)

# Filtrar valores por debajo de un umbral de 50 horas
datos_filtrados_completionist3 <- datos %>% filter(to_beat_completionist <= 50)

# Generar el boxplot con los datos filtrados
ggplot(datos_filtrados_completionist3, aes(x = factor(tags_freq), y = to_beat_completionist)) + 
  geom_boxplot() +
...
Como era esperable, se puede apreciar que hay mucha variedad en las valoraciones de los críticos, ya que cada juego es diferente, sin embargo también es posible observar ciertas tendencias sobre los años y, según los expertos, qué años tuvieron "mejores" salidas en la industria. 

Tomando en cuenta sobre todo los años de los que se tiene más datos así como que es una valoración sobre 100, desde el 2020 se percibe una supuesta mejora en las críticas, indicando que se han sacado al público mejores obras audiovisuales. Por el contrario, desde el 2009 hasta 2013 hubo una peor calidad.
...
La línea roja representa la correlación positiva que hay entre las dos puntuaciones, es decir, que suelen ser muy parecidas en la mayoría de los casos. Además se puede apreciar que la mayoría de los juegos suelen tener una puntuación de entre 5 y el 7.5.
...
datos_filtrados <- datos %>%
  filter(to_beat_main >= 0 & to_beat_main <= 50)
...
Se puede observar en ambos casos que la correlación es muy baja tendiendo a cero por lo que no hay evidencia de que el tiempo en completar el juego principal tenga un impacto significativo en la puntuación de 'metacritic_rating' o 'reviewer_rating'. Ambos gráficos tienen juegos con muy buenas puntuaciones que no tienen muchas horas de juego, lo mismo con muchas horas. No hay un patrón distinguible y se puede concluir que el tiempo no es una variable decisiva. En la última gráfica, aunque no se distingue un patrón claro, se observa una correlación mayor en comparación con la tabla de 'reviewer_rating'. Por lo tanto, se repetirá la gráfica con los 15 tags más frecuentes para verificar si realmente no hay relaciones significativas entre el tiempo necesario para completar el juego y la puntuación en Metacritic. Se espera poder visualizar con mayor claridad cualquier posible relación, aunque esta sea muy débil.

```{r}
# Separar los tags en múltiples filas (asumiendo que están separados por comas)
tags_separados <- datos %>%
  separate_rows(tags, sep = "\\|")

# Contar los 15 primeros tags más comunes
top_15_tags <- tags_separados %>%
  count(tags) %>%
  arrange(desc(n)) %>%
  slice(1:15)

# Filtrar solo los datos que contienen esos 15 tags más frecuentes
datos_filtrados_15 <- tags_separados %>%
  filter(tags %in% top_15_tags$tags)

# Crear un modelo de regresión lineal para cada tag
regresiones_por_tag <- datos_filtrados_15 %>%
  group_by(tags) %>%
  do(modelo = lm(metacritic_rating ~ to_beat_main, data = .))

# Modificar para que el resumen del modelo se almacene como lista
resumen_modelos <- regresiones_por_tag %>%
  summarise(tags, resumen = list(summary(modelo)))

# Crear una lista de los 15 tags más frecuentes
tags_mas_frecuentes <- unique(datos_filtrados_15$tags)

# Iterar sobre cada tag y crear un gráfico individual
for(tag in tags_mas_frecuentes) {
  # Filtrar los datos para el tag actual
  datos_tag <- datos_filtrados_15 %>% filter(tags == tag)
  
  # Ajustar el modelo de regresión
  modelo <- lm(metacritic_rating ~ to_beat_main, data = datos_tag)
  coeficientes <- coef(modelo)
  
  # Crear la fórmula de la línea de regresión
  formula_regresion <- paste0("y = ", round(coeficientes[1], 2), " + ", round(coeficientes[2], 2), " * x")
  
  # Crear el gráfico de dispersión con la regresión lineal
  p <- ggplot(datos_tag, aes(x = to_beat_main, y = metacritic_rating)) +
    geom_point(alpha = 0.5, color = "skyblue") +  # Puntos de dispersión 
    geom_smooth(method = "lm", se = FALSE, color = "coral") +  # Línea de regresión lineal 
    labs(title = paste("Relación entre Tiempo y Metacritic para:", tag),
         x = "Tiempo para completar el juego principal (horas)",
         y = "Puntuación de Metacritic") +
    xlim(0, 50) +  # Limitar el eje X de 0 a 50
    ylim(0, 100) +  # Limitar el eje Y de 0 a 100
    annotate("text", x = 45, y = 5, label = formula_regresion, color = "coral", size = 5, hjust = 1) +  # Añadir la fórmula en la esquina inferior derecha
    theme_minimal()
  
  # Mostrar el gráfico
  print(p)
  
  # Pausa para mostrar cada gráfico sin solaparse
  Sys.sleep(1)  # Pausa de 1 segundo entre gráficas (puedes ajustar)
}
```

Representando cada punto en las gráficas un juego del tag seleccionado y siendo analizado en términos de tiempo y su puntuación por expertos, se puede observar que, aunque hay mucha dispersión en todas, hay una tendencia positiva. Esto sugiere que los juegos que requieren más horas tienden a tener una puntuación ligeramente más alta. Gracias a las fórmulas de las pendientes, es posible afirmar que la pendiente más baja se corresponde con el tag 'Early Access', siendo 0.12, lo que indica que el impacto del tiempo sobre la puntuación es el más pequeño de las 15 gráficas. La pendiente más pronunciada es de 0.33 en el tag 'Puzzle', lo que sugiere que el tiempo de juego tiene un ligero impacto en la puntuación en comparación con el resto de los tags. Además, esta relación tiene sentido si se toma en cuenta el tipo de tag. A pesar de esto, es posible concluir que las relaciones son débiles y que, en general, el tiempo no impacta significativamente en la puntuación del juego.
